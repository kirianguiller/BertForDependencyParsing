...
class PretrainedConfig(object):
    ...
    Args:
        ...
        chunk_size_feed_forward (:obj:`int`, `optional`, defaults to :obj:`0`):
            ...
            # feed-forward-chunking>`__ .
            For more information on feed forward chunking, see `How does Feed Forward Chunking work? <../glossary.html
    ...
    def __init__(self, **kwargs):
        ...
        self.use_cache = kwargs.pop(
            "use_cache", True)  # Not used by all models
        ...
        # Syntactic parsing arguments (added by Kirian)
        self.id2upos = kwargs.pop("id2upos", None)
        self.upos2id = kwargs.pop("upos2id", None)
        if self.id2upos is not None:
            kwargs.pop("num_uposs", None)
            self.id2upos = dict((int(key), value)
                                for key, value in self.id2upos.items())
            # Keys are always strings in JSON so convert ids to int here.
        else:
            self.num_uposs = kwargs.pop("num_uposs", 2)

        self.id2deprel = kwargs.pop("id2deprel", None)
        self.deprel2id = kwargs.pop("deprel2id", None)
        if self.id2deprel is not None:
            kwargs.pop("num_deprels", None)
            self.id2deprel = dict((int(key), value)
                                  for key, value in self.id2deprel.items())
            # Keys are always strings in JSON so convert ids to int here.
        else:
            self.num_deprels = kwargs.pop("num_deprels", 2)

        self.mlp_dropout = kwargs.pop("mlp_dropout", 0.3)
        self.mlp_layers = kwargs.pop("mlp_layers", 2)
        self.mlp_pos_layers = kwargs.pop("mlp_pos_layers", 1)
        self.mlp_input = kwargs.pop("mlp_input", 768)
        self.mlp_arc_hidden = kwargs.pop("mlp_arc_hidden", 500)
        self.mlp_lab_hidden = kwargs.pop("mlp_lab_hidden", 100)
    ...
    @property
    def num_uposs(self) -> int:
        """
        :obj:`int`: The number of uposs for classification models.
        """
        return len(self.id2upos)

    @num_uposs.setter
    def num_uposs(self, num_uposs: int):
        self.id2upos = {i: "upos_{}".format(i) for i in range(num_uposs)}
        self.upos2id = dict(zip(self.id2upos.values(), self.id2upos.keys()))

    @property
    def num_deprels(self) -> int:
        """
        :obj:`int`: The number of deprels for classification models.
        """
        return len(self.id2deprel)

    @num_deprels.setter
    def num_deprels(self, num_deprels: int):
        self.id2deprel = {i: "deprel_{}".format(i) for i in range(num_deprels)}
        self.deprel2id = dict(
            zip(self.id2deprel.values(), self.id2deprel.keys()))
