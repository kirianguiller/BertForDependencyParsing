...
from .configuration_bert import BertConfig  # local import
...
from .modules_utils import MLP, BiAffine
...
class BertForSyntacticParsing(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig):
        ...
        self.num_uposs = config.num_uposs
        self.num_deprels = config.num_deprels
    ...
    def forward(
        ...
        arc_d = self.arc_mlp_d(sequence_output)
        lab_h = self.lab_mlp_h(sequence_output)
        lab_d = self.lab_mlp_d(sequence_output)

        poss_pred = self.pos_mlp(sequence_output)
...
def deprel_aligner_with_head(deprels_pred, heads_true):
    ...
    deprels_pred = torch.gather(deprels_pred, 2, heads_true).squeeze(2)
