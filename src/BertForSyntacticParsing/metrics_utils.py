...
def compute_acc_deprel(deprels_pred, deprels_true, heads_true, eps=1e-10):
    ...
    good_deprel = float(sum(deprels_pred.max(
        dim=1)[1][mask] == deprels_true[mask]))
    # TODO :
...
def compute_LAS_main_aux(heads_pred, deprels_main_pred, deprels_aux_pred, heads_true, deprels_main_true, deprels_aux_true):
    ...
    correct_head = heads_pred.max(dim=1)[1][mask] == heads_true[mask]
    correct_deprel_main = deprels_main_pred.max(
        dim=1)[1][mask] == deprels_main_true[mask]
    correct_deprel_aux = deprels_aux_pred.max(
        dim=1)[1][mask] == deprels_aux_true[mask]

    n_correct_LAS_main = sum(correct_head & correct_deprel_main).item()
    n_correct_LAS_aux = sum(correct_head & correct_deprel_aux).item()
    LAS_epoch = sum(correct_head & correct_deprel_main &
                    correct_deprel_aux).item()
    n_total = float(sum(sum(mask)))
...
def train_epoch(model, n_epoch, train_loader, args):
    ...
    for n_batch, (seq, subwords_start, attn_masks, idx_convertor, poss, heads, deprels_main, deprels_aux) in enumerate(train_loader):
        ...
        seq, attn_masks, heads_true, deprels_main_true, deprels_aux_true, poss_true = seq.to(device), attn_masks.to(
            device), heads.to(device), deprels_main.to(device), deprels_aux.to(device), poss.to(device)

        heads_pred, deprels_main_pred, deprels_aux_pred, poss_pred = model.forward(
            seq, attn_masks)

        loss_batch = 0.0

        loss_head_batch = compute_loss_head(
            heads_pred, heads_true, args.criterions['head'])
        loss_head_epoch += loss_head_batch.item()
        loss_batch += loss_head_batch

        loss_deprels_main_batch = compute_loss_deprel(
            deprels_main_pred, deprels_main_true, heads_true.clone(), args.criterions['deprel'])
        loss_deprels_main_epoch += loss_deprels_main_batch.item()
        loss_batch += loss_deprels_main_batch

        loss_poss_batch = compute_loss_poss(
            poss_pred, poss_true, args.criterions['pos'])
        loss_poss_epoch += loss_poss_batch.item()
        loss_batch += loss_poss_batch

        if args.split_deprel:
            loss_deprels_aux_batch = compute_loss_deprel(
                deprels_aux_pred, deprels_aux_true, heads_true.clone(), args.criterions['deprel'])
...
def eval_epoch(model, eval_loader, args, n_epoch=-1):
    ...
    with torch.no_grad():
        ...
        for n_batch, (seq, subwords_start, attn_masks, idx_convertor, poss, heads, deprels_main, deprels_aux) in enumerate(eval_loader):
            print(
                f"evaluation on the dataset ... {n_batch}/{len(eval_loader)}batches", end="\r")
            # if n_batch>1 : break
            seq, attn_masks, heads_true, deprels_main_true, deprels_aux_true, poss_true = seq.to(device), attn_masks.to(
                device), heads.to(device), deprels_main.to(device), deprels_aux.to(device), poss.to(device)
            ...
            heads_pred, deprels_main_pred, deprels_aux_pred, poss_pred = heads_pred.detach(
            ), deprels_main_pred.detach(), deprels_aux_pred.detach(), poss_pred.detach()
            ...
            for i_vector, (heads_pred_vector, subwords_start_vector, idx_convertor_vector) in enumerate(zip(heads_pred, subwords_start, idx_convertor)):
                ...
                heads_pred_np = heads_pred_vector[:,
                                                  subwords_start_with_root == 1][subwords_start_with_root == 1]
                ...
                try:
                    chuliu_heads_vector = chuliu_edmonds_one_root(
                        np.transpose(heads_pred_np, (1, 0)))[1:]
                except:
                ...
                for i_token, chuliu_head_pred in enumerate(chuliu_heads_vector):
                    ...
                    chuliu_heads_pred[i_vector, idx_convertor_vector[i_token+1]
            ...
            conf_matrix = confusion_matrix(
                deprels_main_pred, deprels_main_true, heads_true, conf_matrix)

            n_correct_LAS_batch, n_correct_LAS_main_batch, n_correct_LAS_aux_batch, n_total_batch = compute_LAS_main_aux(
                heads_pred, deprels_main_pred, deprels_aux_pred, heads_true, deprels_main_true, deprels_aux_true)
            n_correct_LAS_chuliu_batch, n_correct_LAS_chuliu_main_batch, n_correct_LAS_chuliu_aux_batch, n_total_batch = compute_LAS_chuliu_main_aux(
                chuliu_heads_pred, deprels_main_pred, deprels_aux_pred, heads_true, deprels_main_true, deprels_aux_true)

            loss_head_batch = compute_loss_head(
                heads_pred, heads_true, args.criterions['head'])
            good_head_batch, total_head_batch = compute_acc_head(
                heads_pred, heads_true, eps=0)
            loss_head_epoch += loss_head_batch.item()
            good_head_epoch += good_head_batch
            total_head_epoch += total_head_batch

            loss_deprel_main_batch = compute_loss_deprel(
                deprels_main_pred, deprels_main_true, heads_true, args.criterions['deprel'])
            good_deprel_main_batch, total_deprel_main_batch = compute_acc_deprel(
                deprels_main_pred, deprels_main_true, heads_true, eps=0)
            ...
            loss_deprel_aux_batch = compute_loss_deprel(
                deprels_aux_pred, deprels_aux_true, heads_true, args.criterions['deprel'])
            good_deprel_aux_batch, total_deprel_aux_batch = compute_acc_deprel(
                deprels_aux_pred, deprels_aux_true, heads_true, eps=0)
            ...
            good_pos_batch, total_pos_batch = compute_acc_pos(
                poss_pred, poss_true, eps=0)
...
def dep_parse_metrics_computor(eval_prediction: EvalPrediction):
    poss_pred = torch.from_numpy(eval_prediction.predictions[0])
    heads_pred = torch.from_numpy(eval_prediction.predictions[1])
    deprels_main_pred = torch.from_numpy(eval_prediction.predictions[2])

    subwords_start = eval_prediction.label_ids[:, 0, :]
    idx_convertor = eval_prediction.label_ids[:, 1, :]
    poss_true = torch.from_numpy(eval_prediction.label_ids[:, 2, :])
    heads_true = torch.from_numpy(eval_prediction.label_ids[:, 3, :])
    deprels_main_true = torch.from_numpy(eval_prediction.label_ids[:, 4, :])

    n_correct, n_total = compute_LAS(heads_pred,
                                     deprels_main_pred, heads_true, deprels_main_true)
    n_deprel_correct, n_deprel_total = compute_acc_deprel(
        deprels_main_pred, deprels_main_true, heads_true)
    n_head_correct, n_head_total = compute_acc_head(
        heads_pred, heads_true, eps=0)
    n_good_pos, n_total_pos = compute_acc_pos(poss_pred, poss_true)
    return {"LAS": n_correct / n_total, "acc_deprel": n_deprel_correct / n_deprel_total, "acc_head": n_head_correct / n_head_total, "acc_upos": n_good_pos / n_total_pos}
